...
https://github.com/bitnami-labs/sealed-secrets/blob/main/dkubectls/bring-your-own-certificates.md
...
```bash
kubectl apply components/apps/sealed-secrets/operator/base/bootstrap-keys/secret.yaml
```

## Recovery if key gets lost or cluster is created

In case the only lkubectlally stored key gets lost you need to create a new key and afterwards rotate all secrets
...
Without lkubectlal access to cert (Guess its pulling the newest certls and not the one initially created):
```bash
